...
- A body edit reviews only when the PR author edits a body carrying shepherd's `<!-- shepherd:review-changes -->` section: shepherd writes that disclosure after pushing its fixes. Every other title or body edit, a bot's included, is skipped.
- Only runs that review share the PR's concurrency group. A run the job's `if` skips (an ordinary comment, a bot rewriting the PR description after each push) gets a group of its own: in the shared group it cancelled the review in flight after that review had already retracted the approval, so no verdict came. The group's condition is the job's `if` verbatim, and a test keeps them in step.
...
- Every posted review carries `<!-- stamp-run:<ISO start> -->` and `<!-- stamp-reviewed:head=…;base=… -->`. Concurrent runs order themselves from the run marker with no shared state: the run that started later owns the verdict. Retention, the newer-verdict check and the closing sweep read the reviewed marker, never `commit_id`, to know which head one of our reviews covered: GitHub moves a standing approval's `commit_id` forward when the branch is updated from its base, so a carried approval would otherwise look like an approval of the live head.
...
- Reviews and comments authored by our own login are excluded from the prompt, and so is any review carrying our run marker. A previous APPROVED must never be read as independent assurance.
- `isOurs` decides which reviews are ours: our login, or any `[bot]` account carrying our run marker. A GitHub App token posts as `<app>[bot]` while `gh api user` fails, so every sweep, the newer-verdict check and retention match on the marker too. `sweepTargets` is the pure core of the sweep and is tested.
...
- PR content — title, body, diff, file names, comments, reactions, commit messages — is untrusted input everywhere. It sits inside an untrusted-content fence in the prompt, control characters are stripped, and a forged end-of-untrusted sentinel is neutralized before the prompt is built. File names and gate messages are held to one line (`oneLine`) wherever they appear, trusted context included: git allows a newline in a file name, and a new line would read as a trusted fact.
- Review threads come from GraphQL with their resolution, open threads first and newest first; the trusted context counts unresolved threads. shepherd's review-changes section is read out of the body on its own, so a long description never cuts the disclosure. Commit subjects and `Shepherd:` trailers are untrusted claims, never assurance.
- A diff past the prompt's cap is named in the trusted context (the file where it is cut, how many follow), so the reviewer reads the rest or escalates.
- The evidence records `policy_source`: the trusted ref, its commit, and the blob of the policy, guidance and steering that ran, or `bundled` / `absent`. The mechanics footer shows the policy blob.
